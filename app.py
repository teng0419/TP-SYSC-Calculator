...
with st.sidebar.expander("TP-SYSC 高度與角度設定", expanded=True):
    ...
    ic_profile = st.selectbox("選取 IC 段核心斷面", list(CNS_RH_database.keys()), index=list(CNS_RH_database.keys()).index("488 X 300 X 11 X 18") if "488 X 300 X 11 X 18" in CNS_RH_database else 0)
    d_IC, bf_IC, tw_IC, tf_IC = CNS_RH_database[ic_profile]
    ...
    filtered_ej_options = [name for name, (d_v, bf_v, tw_v, tf_v) in CNS_RH_database.items() if (abs(bf_v - bf_IC) <= 20 and d_v >= d_EJ0_min_req - 2.0)]
    if not filtered_ej_options: filtered_ej_options = list(CNS_RH_database.keys())
    ...
    d_EJ0, bf_EJ, tw_EJ, tf_EJ = CNS_RH_database[ej_profile]
...
with st.sidebar.expander("邊界構架尺寸"):
    ...
    rh_beam = st.selectbox("選取邊界梁 RH 尺寸", list(CNS_RH_database.keys()), index=len(CNS_RH_database)-1)
    d_b, bf_b, tw_b, tf_b = CNS_RH_database[rh_beam]
...
W_stiff = (2 * n_h * (d_IC - 2 * tf_IC) * bs * ts + 2 * n_v * h_IC_mm * bs * ts) * rho_steel
...
with tab1:
    ...
    detail_check("未側撐長度 Lb (放寬)", h_SYSC_mm, 10000, "mm", note=r"L_r \text{ 檢核已放寬}")
...
with tab4:
    ...
    # 示意圖 (高對比度專屬配色 - 同構件同色系)
    fig = go.Figure()
    c_flange_ic, c_web_ic = "#FF9F0A", "#FFD60A"
    c_flange_ej, c_web_ej = "#0A84FF", "#5AC8FA"
